...
import { generatePlanCore, generatePlanDays } from './chunkedGeneration.ts';
...
import { parseTripPlanCore } from './tripPlanParts.ts';
...
export async function handleChunkedMode(body: Record<string, unknown>): Promise<ModeResponse> {
  if (body.mode !== 'generate_core' && body.mode !== 'generate_days' && body.mode !== 'finalize_plan') return null;
  ...
  if (body.mode === 'generate_core') {
    const result = await generatePlanCore(parsedRequest.value, rates.rates);
    return result.ok
      ? { body: { core: result.value, elapsedMs: result.elapsedMs }, status: 200 }
      : failure(result.code, result.message, result.status);
  }

  const core = parseTripPlanCore(body.core);
  ...
  const planResult = finalizeGeneratedPlan({ ...core.value, days: body.days }, parsedRequest.value);
