import type { PlannerLanguage, PlannerRequest, TripPlan } from './types.ts';
...
export function applyBudgetWarning(plan: TripPlan, assessment: BudgetAssessment, language: PlannerLanguage = 'ru') {
  if (assessment.level !== 'tight') return plan;
  const warning = language === 'en' ? 'This is a tight budget for the trip. Prioritise more affordable stays, transport, and activities while keeping a small reserve.' : language === 'kk' ? 'Бұл сапар үшін бюджет шектеулі. Шағын қор қалдырып, қолжетімді тұратын орын, көлік және іс-шараларды таңдаған жөн.' : 'Бюджет достаточно плотный для такой поездки. Стоит выбирать более доступное жильё, транспорт и активности, оставив небольшой резерв.';
  const adjustment = language === 'en' ? 'The itinerary was adapted to the upper budget limit, prioritising good-value options.' : language === 'kk' ? 'Жоспар бюджеттің жоғарғы шегіне бейімделіп, үнемді нұсқаларға басымдық берілді.' : 'План адаптирован под верхнюю границу бюджета с приоритетом бюджетных вариантов.';
  return {
    ...plan,
    realism: {
