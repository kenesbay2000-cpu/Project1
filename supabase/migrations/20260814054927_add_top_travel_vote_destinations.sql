drop function public.get_travel_vote_stats();
...
as $$
  ...
  select
    (select count(*) from public.travel_survey_responses),
    (select destination_city from grouped_votes order by vote_count desc, destination_city asc limit 1),
    coalesce((
      select jsonb_agg(
        jsonb_build_object('destination', destination_city, 'votes', vote_count)
        order by vote_count desc, destination_city asc
      )
      from (
        select destination_city, vote_count
        from grouped_votes
        order by vote_count desc, destination_city asc
        limit 4
      ) as top_votes
